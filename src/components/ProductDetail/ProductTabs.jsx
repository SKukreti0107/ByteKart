    ...
    <section className="window-container border-none p-6">
          ...
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-bold ${
              activeTab === tab
                ...
                : 'bg-off-white text-charcoal-dark hover:bg-baby-green'
            }`}
          >
            {tab}
          </button>
      ...
      <ul className="space-y-3">
        {tabContent[activeTab].map((item) => (
          <li key={item} className="flex items-start gap-2 text-sm font-medium text-charcoal-dark/80">
            <span className="material-symbols-outlined text-base text-matcha-deep">check_circle</span>
            {item}
          </li>
        ))}
      </ul>
